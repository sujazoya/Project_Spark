...
public struct SparkDisplayValueData
{
        ...
        double tolerance = 0.000001d)
    {
        ...
        if (useEngineeringPrefixes != other.useEngineeringPrefixes)
            return false;
        ...
        if (double.IsNaN(value) || double.IsNaN(other.value))
            return double.IsNaN(value) && double.IsNaN(other.value);

        if (double.IsInfinity(value) || double.IsInfinity(other.value))
            return value.Equals(other.value);

        double difference = Math.Abs(value - other.value);
